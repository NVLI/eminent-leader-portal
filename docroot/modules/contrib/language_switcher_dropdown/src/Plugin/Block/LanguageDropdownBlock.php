...
class LanguageDropdownBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    if (_lang_dropdown_get_msdropdown_path()) {
      ...
      $form['lang_dropdown']['msdropdown']['#description'] = $this->t('You need to download the <a href=":link">Marghoob Suleman Dropdown JavaScript library</a> and extract the entire contents of the archive into the %path directory on your server.', array(':link' => LANGDROPDOWN_MSDROPDOWN_URL, '%path' => 'sites/all/libraries'));
      $form['lang_dropdown']['msdropdown']['visible_rows'] = array(
        '#type' => 'hidden',
    ...
    if (!$this->moduleHandler->moduleExists('chosen') && _lang_dropdown_get_chosen_path()) {
      ...
      if ($this->moduleHandler->moduleExists('chosen')) {
        ...
        // todo: fix instructions
        $form['lang_dropdown']['chosen']['#description'] = t('You need to download the <a href=":link">Chosen library</a> and extract the entire contents of the archive into the %path directory on your server.', array(':link' => LANGDROPDOWN_CHOSEN_WEB_URL, '%path' => 'sites/all/libraries'));
      }
    }
  ...
  public function blockValidate($form, FormStateInterface $form_state) {
    ...
    switch ($widget) {
      ...
      case LANGDROPDOWN_MSDROPDOWN:
        if (!_lang_dropdown_get_msdropdown_path()) {
          $form_state->setError($form['settings']['lang_dropdown']['widget'], $this->t('You can\'t use <a href=":link">Marghoob Suleman Dropdown</a> output. You don\'t have the library installed.', array(':link' => LANGDROPDOWN_MSDROPDOWN_URL)));
        }
        break;
